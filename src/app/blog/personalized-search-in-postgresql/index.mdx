...
I first presented this approach at [PGConf NYC 2023](https://postgresql.us/events/pgconfnyc2023/schedule/session/1412-postgres-as-personalization-engine/). Having [engineered search infrastructure systems at **Instacart**](https://tech.instacart.com/how-instacart-built-a-modern-search-infrastructure-on-postgres-c528fa601d54), I've seen firsthand how these distributed pipelines crumble under their own weight. Similar patterns power search at giants like [Target](https://cloud.google.com/blog/topics/retail/from-query-to-cart-inside-targets-search-bar-overhaul-with-alloydb-ai) and [Alibaba](https://www.alibabacloud.com/blog/learn-how-alibaba-engineers-accommodated-for-face-masks-in-their-face-recognition-algorithms_596040).
...
<Note>
This post is based on a [functional prototype concept](https://github.com/ankitml/paradedb-reranker) I created which demonstrates in-database personalization patterns. If you're the type who reads the last page of a novel first, feel free to start there.
</Note>
...
1. **Retrieval**: We use [**BM25**](https://www.paradedb.com/learn/search-concepts/bm25) (standard full-text search) for lexical search to find the top N candidates (100 here) matching the user's query. This is extremely fast and computationally cheap, filtering millions of rows down to a relevant subset. You can [combine BM25 and Pgvector for a hybrid retrieval](https://www.paradedb.com/learn/search-concepts/hybrid-search).
...
Why this approach? Because running vector similarity search on your _entire_ dataset for every query is overkill. For $$n=100$$ items (or even 1000), we would be better to ignore vector indexes, and instead brute force our way to a result.
...
A powerful alternative is using a **Cross-Encoder**. Our in-database approach uses a **Bi-Encoder**, which calculates embeddings for the query and document _independently_ and compares the distance between them. A Cross-Encoder, by contrast, feeds _both_ the query and document into the model simultaneously.

- **Pro**: **Maximum Accuracy**. Cross-encoders are more advanced because they understand the interaction between words in the query and words in the document, though they are generally considered a black box for most users.
