...
Benchmarking boils down to two concerns: the workload (what you measure) and the execution (how you measure it). Each is harder than it looks, and most published benchmarks underweight at least one.

The two shape each other, so where do you start? Do you begin with the workload you want to highlight and build the execution around it? Or do you build the execution first and let the workloads come later? This post is about the path we picked, and the runner we built around it.
...
Here's how the pieces fit together: backends, datasets, the script you write, and the dashboard.
