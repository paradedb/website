...
There a few ways to get BM25 in Postgres, but the easiest, fastest, and most scalable is through the ParadeDB pg_search extension.
...
The technique that makes this work elegantly is [Reciprocal Rank Fusion (RRF)](../../learn/search-concepts/reciprocal-rank-fusion). RRF takes multiple ranked lists and merges them by rewarding documents that appear near the top of any list. This is needed because although BM25 and pgvector score documents, they don't do it on the same scale (in fact BM25 ranking are entirely corpus dependent and can't be compared cross-indexes either).
