...
Our search analytics are powered by a new [`pdb.agg()`](https://www.paradedb.com/docs/reference/aggregates/overview) function can be used in two ways:
...
`pdb.agg()` can also be used with a [JSON argument](https://www.paradedb.com/docs/reference/aggregates/overview) that closely mirrors the [Elasticsearch aggregations API](https://www.elastic.co/docs/explore-analyze/query-filter/aggregations), and in many cases is mapped to plain SQL constructs (like `COUNT(*)`).
...
Keep an eye out for another post showing off all the new v2 API features. For now, you can explore the full API reference in our [documentation](https://www.paradedb.com/docs/reference/full-text/overview).
...
Ready to see search aggregations and the new API in action? [Try ParadeDB 0.20.0](https://www.paradedb.com/docs/start/install) and experience what it's like when search, analytics, and transactions work together seamlessly.
