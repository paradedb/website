...
To demonstrate this we ran one TopK full-text search query over one million Hacker News rows against ParadeDB and Postgres FTS across three passes, while the dataset, query shape, hardware, and backend setup stay fixed. Pass 1 is the baseline: single hardcoded term, closed loop. Pass 2 changes the **workload** to a forty-term rotation across a wider selectivity range. Pass 3 keeps that workload and changes the **execution model** to a fixed-rate open loop. Each pass produces a different and equally true answer; the point is not which one is right, but that each axis is a separate methodology decision, and exploring all three was easy.
...
ParadeDB indexes `text` directly with [BM25](https://paradedb.com/learn/search-concepts/bm25). Postgres pre-tokenizes `text` into a stored generated `tsv` column and indexes that value with GIN[^3]. Both are configured for English text processing, and both answer the same query shape using their native syntax:
...
ParadeDB's BM25 implementation is built on Tantivy, which uses a [block-max-WAND-style top-K traversal](https://paradedb.com/learn/search-concepts/block-wand). The index keeps per-block score upper bounds so the reader can skip whole blocks that cannot contribute to the top ten. We've written about how this plays out in Postgres in [How We Optimized Top K in Postgres](https://www.paradedb.com/blog/optimizing-top-k). In this run, that kept the higher selectivity terms fast while Postgres FTS spent most of its time ranking the larger result sets.
