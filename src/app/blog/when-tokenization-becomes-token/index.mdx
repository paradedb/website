...
<TokenizerDemo mode="character-filtering" defaultText="The full-text database jumped over the lazy café dog"/>

Of course there are a lot more filters which may happen here, but for the sake of brevity lets move on.
...
`Jump`, `jumps`, `jumped`, `jumping`. Humans see the connection instantly. Computers don't, unless we give them a way [^1].
...
<TokenizerDemo mode="stemming" defaultText="full text database jumped lazy cafe dog" />
...
<TokenizerDemo
  ...
  defaultText="databas jump lazi cafe dog"
/>
...
[^1]: Another way is vector search, which trades lexical stemming for searching over sematic meaning.
