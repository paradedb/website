...
The standard RRF formula treats all ranking sources equally:

$$
\text{RRF}(d) = \sum_{r \in R} \frac{1}{k + rank_r(d)}
$$

However, **Weighted RRF** allows different ranking sources to have different influence, which is often more effective:
...
A variety of databases and search enginges have pre-implemented RRF, but it's a very simple formula and can be easily demonstrated in SQL as follows:
...
-- Sum the rrf scores, order by them, and join back the orginal data
SELECT
  m.id,
  SUM(s) AS score,
  m.description
