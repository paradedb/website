...
1. **Retrieval**: Information is retrieved from external data sources. Usually the user query will be used, wth the RAG pipeline employng vector search or full-text search to surface relevant information. 
...
Most AI models convert inputs into multi-dimensional vectors known as **embeddings**. Two pieces of information that the AI model considers interrelated will produce proximate embeddings relative to other unrelated information. For example, `"yellow cat"` and `"orange leopard"` would render more similar embeddings than the embeddings of `"database index"` or `"full-text search"`. 

[Vector search](/learn/search-concepts/vector-search) is ideal for RAG systems that must retrieve semantically similar information (e.g. `"full-text search"` and `"lexical processing"`). It is also ideal for multimodal applications (e.g. those that generate images or video) as multimodal AI models also produce embeddings. 
...
Once you retrieve relevant information, you need to combine it with the user's original query before sending it to the AI model. This augmentation step determines how effectively the model can use the retrieved context.
