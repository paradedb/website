...
At query time, performance depends on the index staying in cache. Size `shared_buffers` and OS cache so the working set of the HNSW graph stays resident. A vector index that spills to disk has a long latency tail no parameter will fix. See [Tuning PostgreSQL for Performance](/learn/postgresql/tuning-postgresql) for the general memory configuration this builds on.

## Tuning Filtered Searches

Filtered vector searches may need to scan more candidates after PostgreSQL applies `WHERE` predicates. Start with the query-time recall knobs, then add scan limits only when you need tighter control:
...
Use these settings with measured recall. Iterative scans can recover missing filtered results by scanning more of the index, but they also increase CPU, memory, and latency. HNSW supports `strict_order` and `relaxed_order`; IVFFlat supports `relaxed_order`.
