...
pgvector evaluates the ANN index and then applies the filter, which can return fewer than `LIMIT` rows if the filter is selective. This behavior is one of the main practical differences between introductory vector search examples and production retrieval systems. The [limitations](/learn/postgresql/pgvector-limitations) and [production](/learn/postgresql/pgvector-in-production) guides cover how to handle selective filters at scale.
