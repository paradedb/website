...
Modern AI applications produce embeddings: fixed-length arrays of floating-point numbers that represent the semantic content of text, images, or audio. Similar inputs map to vectors that sit near each other in this space, so retrieval becomes a [vector search](/learn/search-concepts/vector-search) problem rather than a keyword match. This is the core difference between [lexical and semantic search](/learn/search-concepts/lexical-vs-semantic). [pgvector](https://github.com/pgvector/pgvector) is a PostgreSQL extension that stores and searches those embeddings with a native `vector` data type, distance operators, and approximate nearest neighbor indexes. It does not create embeddings itself; you generate them with an embedding model, then insert the resulting vectors into PostgreSQL.

Use pgvector when your vectors need to live next to relational data: users, permissions, tenants, products, documents, timestamps, or workflow state. It is a good fit when you want semantic search inside an application that already uses PostgreSQL, especially if you need SQL filters, joins, transactions, backups, and access control around the same data. A separate vector database is more likely to make sense when vector search is the main workload, the corpus is very large, or you need specialized retrieval features outside PostgreSQL.
...
Four distance operators cover the common similarity measures. The right one depends on how your embeddings were trained and normalized:
