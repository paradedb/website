...
Modern AI applications produce embeddings: fixed-length arrays of floating-point numbers that represent the semantic content of text, images, or audio. Similar inputs map to vectors that sit near each other in this space, so retrieval becomes a [vector search](/learn/search-concepts/vector-search) problem rather than a keyword match. This is the core difference between [lexical and semantic search](/learn/search-concepts/lexical-vs-semantic).
