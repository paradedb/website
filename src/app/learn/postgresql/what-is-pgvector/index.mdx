...
- `halfvec` stores each component as a 16-bit float, halving size
- `bit` stores binary vectors for Hamming and Jaccard distance
- `sparsevec` stores only nonzero components, useful for sparse embeddings

Both IVFFlat and HNSW can be built on these lower-precision or sparse types, so you can fit larger indexes in memory while keeping the same nearest-neighbor query shape.
