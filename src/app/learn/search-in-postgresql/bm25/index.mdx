...
There are many ways to implement BM25 in Postgres. Some are significantly more suitable for scaling than others. To begin, let's start with the most rudimentary approach: using just SQL. 
...
A more mature path to implementing BM25 is to use auxiliary tables. With each term occupying a row, index lookups are reduced to a single term per query. Then, these terms could be aggregated with a custom function.
...
plpgsql_bm25 includes an optional Python implementation that can rank things upwards of 30-40x faster. The library's Python inclusion reflects that SQL + auxiliary tables remain slow. Raw Postgres isn't designed for these "calculate, then aggregate" workflows. It's a database oriented around OLTP correctness and flexibility, not number crunching. 

To fix this, a more hybridized approach is needed. ParadeDB takes this path. 

### Use BM25 on ParadeDB, a Postgres database

ParadeDB is an open-source Postgres database with built-in BM25 search, powered by the `pg_search` extension.
...
ParadeDB's efficiency stems from its underlying memory structure: an LSM tree composed of inverted index and columnar index branches. The branch structure makes ParadeDB extremely efficient for reads, while the tree structure accommodates heavy writes. This structure enables parallelization; the BM25 index significantly outpaces traditional Postgres's Full-Text Search. 
...
Keeping search functionality within the database avoids ETL pipelines where data syncs from Postgres to external search engines, reducing points of failure, latency, and stale content risks. 

Stale content becomes problematic at scale. Applications using BM25 to rank search results may return outdated information when external search engines lag behind database updates. External search engines with lazy updates can surface inconsistent data. 
