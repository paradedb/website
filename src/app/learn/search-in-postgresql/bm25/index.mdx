...
The ParadeDB index is laid out on disk as an [**LSM tree**](https://www.paradedb.com/docs/concepts/architecture#lsm-tree), where each segment in the tree consists of both an inverted index and columnar index. The inverted and columnar indexes optimize for fast reads, while the LSM tree optimizes for high-frequency writes.
