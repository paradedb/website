...
=======
That's not to say that the built in features aren't great, but for when you need a streamlined developer experience, [BM25](/learn/search-concepts/bm25) ranking, a feature set that matches Elasticsearch, or increased performance then ParadeDB is a better fit.
>>>>>>> c86d7ce (Fix and link)
...
<Note>
  Another option is ParadeDB, which uses an extension to bring the feature set
  and performance of Elasticsearch inside Postgres.
</Note>
