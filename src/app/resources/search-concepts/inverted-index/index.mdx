...
A **forward index** maps documents → terms. It’s easy to build but terrible for search. To find docs containing “cat,” you’d scan every document’s term list.

An **inverted index** maps terms → documents. More work up front, but queries are lightning-fast.

Both structures are often kept together: forward indexes for reconstructing results, inverted indexes for finding them. This mirrors the tradeoffs databases make with B-trees and hash indexes: storage overhead for massive query-time speed.
...
## Multi-Term Query Example

Suppose a user searches for `"quick brown"`.
...
Intersection = [1, 3]. Both docs are candidates.

If the query is `"quick fox"`:

- `quick` → [1, 3]
- `fox` → [1]

Intersection = [1]. Only Doc 1 matches.

This shows why inverted indexes are so efficient: queries reduce to set operations on integers, not text scanning.
