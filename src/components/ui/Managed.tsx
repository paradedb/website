  ...
  return (
    ...
    >
      ...
      <p className="mt-2 max-w-2xl text-gray-600 md:mt-6 md:text-lg">
        ParadeDB powers search and analytics for some of the most demanding enterprise applications.
      </p>
      <div className="mt-12 grid grid-cols-2 gap-x-8 gap-y-4">
        {caseStudies.map((study, index) => (
          <div
            key={index}
            className="relative col-span-2 h-max max-w-2xl animate-slide-up-fade rounded-2xl sm:ml-auto sm:w-full md:col-span-1 group"
            style={{ animationDuration: "1400ms" }}
          >
            <Link href={`/blog/${study.link}`} target="_blank">
              <div
                className={classNames("rounded-xl relative overflow-hidden", study.bgStyle, "py-8 px-8 hover:cursor-pointer duration-300")}
              ...
              >
                ...
                <div className="relative z-10">
                  <study.logo />
                  <div className={classNames("text-6xl md:text-8xl font-medium", study.textStyle, "mt-12 md:mt-20")}>
                    {study.metric}<span className="text-5xl md:text-7xl font-normal">{study.unit}</span>
                  </div>
                  <p className={classNames("mt-2", study.textStyle)}>
                    {study.description}
                  </p>
                  <Button
                    className={classNames("group mt-12 md:mt-16 bg-transparent hover:bg-transparent px-0", study.textStyle)}
                    variant="light"
                  >
                    Read Story
                    <ArrowAnimated
