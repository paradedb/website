...
import Headshot from "../../../public/blog/phil_headshot.png";
import SplashImage from "../../../public/blog/etl_banner.svg";
import EtlExample from "../../../public/blog/etl_pipeline_example.svg";
import LrExample from "../../../public/blog/logical_replication_example.svg";
...
<div className="relative bottom-10 flex h-6 space-x-3">
  ...
  <p className="relative bottom-1.5">
    By Philippe Noël and Mathew Pregasen on August 8, 2025
  </p>
</div>

<Image src={SplashImage} className="rounded-xl mt-12 md:mt-6" />
...
1. **Incompatible with non-Postgres sinks**: Logical replication only works between Postgres instances. There’s no built-in way to stream changes to non-Postgres systems, which is where CDC-based (change data capture) ETL tools shine.
2. **Schema coupling limits flexibility**: Aside from the ability to specify [row filters](https://www.postgresql.org/docs/current/logical-replication-row-filter.html) and [column lists](https://www.postgresql.org/docs/current/logical-replication-col-lists.html), the replica must mirror the source schema exactly. There’s no sophisticated transformation layer to reshape data for any downstream need.
3. **Single-thread apply process limits throughput**<sup>1</sup>: Each subscription applies changes in a single thread, regardless of how many tables are replicated inside a subscription. A common pitfall is placing too many tables into one subscription instead of splitting across multiple subscriptions.
4. **It doesn’t replicate DDL**<sup>2</sup>: Only row-level changes (INSERT, UPDATE, DELETE) are replicated—schema changes like `CREATE INDEX` or `ALTER TABLE` are not. The workaround is to run the same migration scripts on both your primary Postgres and the ParadeDB replica.

## What About Postgres Wire-Compatible Databases?

Many modern databases — like ClickHouse and others — claim compatibility with the Postgres wire protocol. At first glance, this seems like a way to bypass ETL entirely. If these systems can speak Postgres, why isn’t that enough?

The catch is that these databases have a different internal data model than Postgres, so they still need to use ETL to reshape the incoming data to fit. You’re still dealing with type mismatches, edge cases, and implicit transformation logic. And since these systems don’t use MVCC (Multi Version Concurrency Control) like Postgres, you lose the built-in conflict handling Postgres provides.

So yes, Postgres wire protocol compatibility is certainly nice. But the pitfalls of ETL persist.

## Which Databases are Compatible with Logical Replication?

Traditionally, organizations have spun up Postgres logical replicas tuned for specific workloads by customizing Postgres indexes, memory settings, or using different instance types. This improves performance and reduces contention on the primary, but only up to a certain point.

Today, many teams go a step further by replicating into specialized Postgres variants built for particular workloads.
...
For instance, [ParadeDB](https://paradedb.com) is an Elasticsearch alternative built on Postgres. It’s designed for Elastic-style search and analytical queries and is fully compatible with Postgres logical replication.
