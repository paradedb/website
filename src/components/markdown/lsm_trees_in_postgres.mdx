...
<div className="relative bottom-10 flex h-6">
  <Image src={StuHeadshot} className="h-7 w-7 rounded-full z-50" />
  <Image
    src={MingHeadshot}
    className="h-7 w-7 rounded-full z-10 relative right-2"
  />
  <div className="relative top-8 h-7 w-7 rounded-full bg-indigo-600 border-2 border-indigo-300 text-white text-xs text-center p-0 right-4 font-semibold">
